...
function IncidentCard({ incident, index, isInGroup = false, onClick }: { incident: Incident; index: number; isInGroup?: boolean; onClick?: () => void }) {
  ...
  const handleSourcesClick = (e: React.MouseEvent) => {
    // Prevent card click when clicking on source links
    e.stopPropagation()
  }
  ...
  return (
    <motion.article
      ...
      aria-label={onClick ? `View details for incident: ${incident.title}` : undefined}
    >
      {/* Header Section */}
      ...
      {hasUnverifiedSources ? (
        ...
        <div onClick={handleSourcesClick}>
